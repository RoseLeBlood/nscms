<?php 
    require_once dirname(__FILE__) .  '/nc.module.ressourcen.list.php';
    ...
    class nc_module_system extends nc_object {
        ...
        public static function instance($parent = null) {
            if(self::$m_pInstance == null) 
                self::$m_pInstance = new nc_module_system($parent);
            return self::$m_pInstance;
        }
        private function __construct($parent) {
            nc_object::__construct($parent);
            
            $this->m_ncRessourcenList = new nc_module_ressourcen_list($parent);
        ...
        public function get_object($name, $parent) {
            if(!isset($this->m_ncLoadedModules[$name]))  {
                ...
                for($i = 0; $i < $_list->get_count(); $i++) {
                    ...
                    if($_item->get_name() == $name AND $_item->is_active()) {
                        ...
                        $this->m_ncLoadedModules[$name] = new $_name ($_item, $parent);
