<?php 
    ...
    class nc_visual_links extends nc_visual_object {
        public function __construct($path, $item) {
            nc_visual_object::__construct( new nc_base_file($path . '/../../htm/nc.link.htm') );
    ...
    class nc_links_widget_content extends nc_visual_object {
        public function __construct($path, $linkTable) {
            nc_visual_object::__construct(null);
            
            for($i = 0; $i < $linkTable->get_count(); $i++) {
                $item = $linkTable->get_item($i);
                $this->m_arrayItems[] = new nc_visual_links($path, $item);
            } 
        }
        public function setvar() {
